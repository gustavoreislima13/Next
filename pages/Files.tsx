...
export const Files: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Arquivos</h1>

      {/* Upload Area */}
      <div 
        className={`
          border-2 border-dashed rounded-xl p-10 text-center transition-all duration-300 cursor-pointer hover:shadow-lg hover:-translate-y-1
          ${dragActive 
      ...
      >
        ...
        <p className="text-lg font-medium text-slate-700 dark:text-slate-300">Arraste e solte arquivos aqui</p>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">ou clique para selecionar do computador</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800">
          <div className="relative max-w-md">
             ...
             <input 
              ...
              className="w-full pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:border-blue-500 bg-white dark:bg-slate-950 text-slate-900 dark:text-white"
              value={search}
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {filteredFiles.map(file => {
             ...
             return (
              <div key={file.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 gap-4 transition-colors">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-500 dark:text-slate-400 flex flex-wrap gap-2">
                      ...
                      {(associatedClientName || associatedTx) && (
                        <div className="flex items-center gap-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-1.5 py-0.5 rounded border border-blue-100 dark:border-blue-800">
                          <Link size={10} />
                ...
                <div className="flex items-center gap-2 self-end sm:self-auto">
                  <button className="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 text-sm font-medium px-2">Download</button>
                  <button 
                    onClick={() => openEditModal(file)}
                    className="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 p-1 rounded-md hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors"
                  ...
                  <button 
                    ...
                    className="text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 p-1 rounded-md hover:bg-rose-50 dark:hover:bg-rose-900/30 transition-colors"
      ...
      {editingFile && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in border border-slate-200 dark:border-slate-800">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Editar Arquivo</h2>
            ...
            <form onSubmit={handleUpdateFile} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Vincular Transação</label>
                <select 
                  className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-950 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                ...
                {editFormData.associatedClient && (
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Filtrando transações do cliente selecionado.</p>
                )}
              </div>
              
              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={closeEditModal} className="px-4 py-2 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-sm font-medium">Cancelar</button>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-fade-in p-6 border border-slate-200 dark:border-slate-800">
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Excluir Arquivo</h3>
